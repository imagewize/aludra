...
The script lives in wp-ops rather than in each package: its paths are personal configuration, not package code, and a `.sh` file committed to a theme fails WordPress Theme Check outright. Elayne and Nynaeve keep their copies untracked for the same reason; `bin/sync-demo.sh` is gitignored here if you want a local shortcut.
...
- Run one-off commands non-interactively: `trellis vm shell --workdir /srv/www/<site-key>/current -- <command>` (site key from `wordpress_sites.yml`, e.g. `demo.imagewize.com` or `imagewize.com`; VM path is always `/srv/www/<site-key>/current`). Example: `wp post get 6765 --field=post_content`. **`trellis vm shell` does not run the `<command>` string through a shell** — `&&`, `||`, pipes and redirects are passed through literally and fail (e.g. `"echo one && echo two"` errors with `command not found` on the whole literal string). For anything needing real shell chaining, call `limactl shell` directly instead — it accepts the same `--workdir` flag and does honour `bash -c '...'`: `limactl shell --workdir /srv/www/<site-key>/current <instance-name> -- <command>` (instance name from `limactl list`, usually matching the Trellis project's domain, e.g. `imagewize.com`). Flag order is also more forgiving through `limactl shell` directly (`--url=` can come before or after the subcommand); through the `trellis vm shell` wrapper keep `--url=` last, as flags before the subcommand can get swallowed.
...
- **Syncing plugin/theme files is not enough to test a pattern change.** `wp-ops rsync-package-to-site` updates the files WordPress reads a *pattern* from, but any page already created from that pattern keeps its own copy of the markup in `post_content` — it never re-reads the pattern after insertion. If a demo page predates the pattern edit you're testing (compare `post_modified` to the commit date), the fix's CSS selector may target a class or attribute the live page's stored markup simply doesn't have yet, and the bug will look unfixed even though the code is correct. Check first with `wp post get <id> --field=post_content --url=...`; if it's stale, rebuild it from the current pattern rather than hand-editing markup — `~/code/imagewize.com/demo/scripts/rebuild-demo-subsite.php` (a `wp eval-file` script, maintained in the `imagewize.com` repo, not here) does this for the subsites it already maps and is the fast, repeatable way to do it for one more page: pass the pattern's registered slug to `WP_Block_Patterns_Registry::get_instance()->get_registered( $slug )` and feed `$pattern['content']` straight into `wp_update_post()` for that post ID.
...
### Bump the block's own version too

Every block carries its own `version` in `block.json`, independent of the plugin version. **When
you change a block's markup, styles, or attributes, bump that block's `version` as well** — patch
for a style or rendering fix, minor for a new attribute or style variation.

It has to be bumped in **both** copies, or the two disagree:

```
blocks/<block>/src/block.json     # source of truth — edit this
blocks/<block>/build/block.json   # committed build output — regenerated by npm run build
```

`npm run build` copies `src/block.json` to `build/`, so building after the edit keeps them in
step; if you hand-edit, edit both. `register_block_type()` reads the **build** copy, and WordPress
uses that `version` to cache-bust the block's enqueued script and style — a style fix shipped
without the bump can keep serving the old CSS from a browser or CDN cache.

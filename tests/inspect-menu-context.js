...
async function inspectMenuContext() {
  ...
  try {
    ...
    const contextData = await page.evaluate(() => {
      const megaMenuBlock = document.querySelector('[data-wp-interactive="aludra/mega-menu"]');
      if (!megaMenuBlock) {
      ...
      // Get panel classes
      const panel = megaMenuBlock.querySelector('.wp-block-aludra-mega-menu__panel');
    ...
    await page.click('.wp-block-aludra-mega-menu__toggle');
    await page.waitForTimeout(600);

    const runtimeState = await page.evaluate(() => {
      const panel = document.querySelector('.wp-block-aludra-mega-menu__panel');
      if (!panel) {
    ...
    await page.screenshot({
      path: '/Users/jasperfrumau/code/aludra/screenshots/context-inspection.png',
      fullPage: false
    });

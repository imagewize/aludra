...
- "Disable All" skipped dependency/disabled-state styling for child block checkboxes.
- Carousel/Testimonial Grid asset loading used an incomplete settings default, which could skip loading Slick Carousel assets in an edge case where the option was set without a `carousel` key.

...
Add a WordPress admin panel to allow users to selectively enable/disable individual blocks in the Aludra plugin.

**Available Blocks:**
1. **Carousel** (aludra/carousel) - Parent block for slides
2. **Slide** (aludra/slide) - Child block, requires Carousel
3. **Mega Menu** (aludra/mega-menu) - Standalone navigation block
4. **FAQ Tabs** (aludra/faq-tabs) - Parent block for FAQ answers
5. **FAQ Tab Answer** (aludra/faq-tab-answer) - Child block, requires FAQ Tabs
6. **Search Overlay Trigger** (aludra/search-overlay-trigger) - Standalone search block

## Current Architecture

The plugin uses **dynamic block discovery** (`aludra.php:31-55`):
...
1. **Settings page** in WordPress admin under Settings → Aludra
...
**Key functions:**
```php
aludra_add_settings_page()     // Hook into admin_menu
aludra_register_settings()     // Register settings with Settings API
aludra_settings_page_html()    // Render settings page HTML
aludra_settings_section_cb()   // Section callback
aludra_settings_field_cb()     // Field callback for each block
```

**Option name:** `aludra_enabled`
**Data structure:**
```php
array(
    'carousel' => true,
...
function aludra_validate_dependencies( $enabled_blocks ) {
...
├── aludra.php            # Modified registration logic
└── assets/
    └── admin/
...
2. **Nonce verification** - Protect settings form
   ```php
   check_admin_referer( 'aludra_settings' );
   ```

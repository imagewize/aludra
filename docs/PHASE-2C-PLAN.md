...
.wp-block-aludra-mega-menu {
  ...
  @media (max-width: 768px) {
    ...
    &--layout-grid {
      .wp-block-aludra-mega-menu__panel__content {
...
export default function Save({ attributes }) {
  ...
  const blockProps = useBlockProps.save({
    className: classnames(
      `wp-block-aludra-mega-menu--layout-${layoutMode}`,
      {
        'has-hover-activation': enableHoverActivation &&
          (layoutMode === 'dropdown' || layoutMode === 'grid'),
      }
    ),
    'data-wp-interactive': 'aludra/mega-menu',
  ...
  return (
    <div {...blockProps}>
      {/* Trigger button */}
      <button
        className="wp-block-aludra-mega-menu__trigger"
      ...
      >
        ...
        <span className="wp-block-aludra-mega-menu__label">{label}</span>
      </button>
      ...
      >
        {layoutMode === 'grid' ? (
          <div className="wp-block-aludra-mega-menu__panel__content">
            <InnerBlocks.Content />
          </div>
        ) : (
          <InnerBlocks.Content />
      ...
      {(layoutMode === 'overlay' || layoutMode === 'sidebar') && (
        <div
          className="wp-block-aludra-mega-menu__backdrop"

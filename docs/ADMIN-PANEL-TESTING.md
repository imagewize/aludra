...
- [ ] **Settings page appears under Settings menu**
  ...
  - Verify "Aludra" menu item appears
...
- [ ] **Existing sites with no saved settings see all blocks enabled**
  - Delete the `aludra_enabled` option from database:
    ```sql
    DELETE FROM wp_options WHERE option_name = 'aludra_enabled';
    ```
  - Visit a page with existing blocks
  - Verify all blocks still work
  - Visit Settings → Aludra
  - Verify all checkboxes are checked by default

### UI/UX

- [ ] **Settings page displays properly**
  - Verify page title shows "Aludra Settings"
...
console.log('Checkboxes:', $('.aludra-block-checkbox').length);
// Expected: 6

// Test 2: Check dependency data attributes
console.log('Slide parent:', $('#aludra_slide').data('parent'));
// Expected: "carousel"

console.log('FAQ Tab Answer parent:', $('#aludra_faq-tab-answer').data('parent'));
// Expected: "faq-tabs"

// Test 3: Test Enable All function
$('#aludra-enable-all').click();
console.log('All checked:', $('.aludra-block-checkbox:checked').length);
// Expected: 6

// Test 4: Test Disable All function
$('#aludra-disable-all').click();
console.log('All unchecked:', $('.aludra-block-checkbox:not(:checked)').length);
// Expected: 6
```
...
SELECT * FROM wp_options WHERE option_name = 'aludra_enabled';

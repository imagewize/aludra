...
/**
 * Testimonial Grid Block - Frontend Initialization
...
( function ( $ ) {
	$( document ).ready( function () {
		$( '.wp-block-aludra-testimonial-grid' ).each( function () {
			const $block = $( this );
			...
			const slickSettings = JSON.parse(
				$block.attr( 'data-slick' ) || '{}'
			);
			...
			function wrapSlidesForCarousel() {
				if (
					! $block.find( '.testimonial-grid__slides-wrapper' ).length
				) {
					...
					const $cards = $block.children(
						'.testimonial-grid__card, .wp-block-group'
					);

					// Create wrapper and move cards into it
					const $wrapper = $(
						'<div class="testimonial-grid__slides-wrapper"></div>'
					);
			...
			function initCarousel() {
				if ( shouldEnableCarousel() ) {
					...
					if ( ! $wrapper.hasClass( 'slick-initialized' ) ) {
						...
						const slideSpacing =
							$block.data( 'slide-spacing' ) || 12;
						$wrapper.css( {
							'--arrow-color': $block.data( 'arrow-color' ),
							'--arrow-background':
								$block.data( 'arrow-background' ),
							'--arrow-hover-color':
								$block.data( 'arrow-hover-color' ),
							'--arrow-hover-background': $block.data(
						...
						const dotsBottom =
							$block.data( 'dots-bottom' ) || '-45px';
						setTimeout( () => {
							$wrapper
								.find( '.slick-dots' )
								.css( 'bottom', dotsBottom );
						}, 50 );
					...
					const $wrapper = $block.find(
						'.testimonial-grid__slides-wrapper'
					);
					if (
						$wrapper.length &&
						$wrapper.hasClass( 'slick-initialized' )
					) {
						// Destroy carousel if conditions no longer met
						$wrapper.slick( 'unslick' );
						$block.removeClass(
							'testimonial-grid--carousel-active'
						);
					}
				}
			}
